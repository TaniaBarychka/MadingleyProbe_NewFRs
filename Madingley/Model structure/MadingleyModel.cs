...
{   
    ...
    public class MadingleyModel
    {
        ...
        //My array collecting array
        RogierDataCarriers dataobj;
            ...
            string outputFilesSuffix, string globalModelTimeStepUnit, int simulation)
        {
            ...
            // Set up the Rogier's stuff AFTER everything else is initialised:
            int nhigh = (int)EcosystemModelGrid.NumLatCells;
            int nwide = (int)EcosystemModelGrid.NumLonCells;
            int numberfunctgroups = CohortFunctionalGroupDefinitions.GetNumberOfFunctionalGroups();

            //THE ONLY hard-coded PARAMETER... THIS SHOULD REALLY BE READ IN AS A SIMULATION PARAMETER...
            int rogierburnin = 100;

            // Make all the necessary arrays
            dataobj = new RogierDataCarriers(nhigh, nwide, numberfunctgroups, CohortFunctionalGroupDefinitions,rogierburnin); 
            
            // Finally, set up the sqlite database.
            dataobj.CreateDB();
            // Rogier stuff set up. 
        ...
        {            
            ...
            Boolean varExists;
             ...
             for (UInt32 hh = 0; hh < NumTimeSteps; hh += 1)
             {
                 ...
                 Console.WriteLine("Of year {0}...", year);
                 ...
                 //ROGIER RECORD TSTART DATA HERE.
                 if (year >= dataobj.burnintime)
                 {
                     Console.WriteLine("I AM COLLECTING!!!!!");
                     dataobj.TestZeroedAbund();
                     dataobj.CollectTstartData(_CellList, EcosystemModelGrid, hh);
                 }
                 //END ROGIER
                 ...
                 if (year >= dataobj.burnintime)
                 {
                     //Now collect abundance data post cohort merging for the calculation of dispersal rates.
                     dataobj.CollectDispAbundData(_CellList, EcosystemModelGrid, hh);

                     //Work out all the vital rates, using the pre-merging abundances.
                     dataobj.CalculateVitalStatistics(_CellList, hh);
                 } 
                //Rogier end
                 ...
                 //Rogier Edit DATA IS COLLECTED AND CALCULATED IN FUNCTION BELOW
                 ...
                 if (year >= dataobj.burnintime)
                 {
                     // you can finally zero out all of the carrier arrays.
                     // Increment number of time steps
                     dataobj.recordedtime += 1;
                 }
                ...
                GC.Collect();
                 ...
                 Console.WriteLine("Rogier's collecting and madingley timestep COMPLETE!");
            ...
            //Rogier edit:
            //NOW WRITE TO DISC
            Console.WriteLine("Preparing to write to disc");

            // Count the dataset
            int[] totaldata = dataobj.CountDataset();
            
            // Add the dataset.
            dataobj.globaln = (int)NumTimeSteps;
            dataobj.AddDataToDatabase(totaldata, (int)CellSize, (int)LeftmostLongitude, (int)BottomLatitude, (int)RightmostLongitude, (int)TopLatitude);
            //Rogier finish.
        ...
        //ROGIERS VERSION, SANS MERGE, OF THE RUN CELL ALGORITHM! REQUIRES 2 FUNCTIONS: RunCellNoMerge AND RunWithinCellCohortEcologyNoMerge 
        public void RunCellNoMerge(int cellIndex, ThreadLockedParallelVariables partial, Boolean dispersalOnly,
           MadingleyModelInitialisation initialisation)
        {
            // Apply any climate change impacts
            ClimateChangeSimulator.ApplyTemperatureScenario(
           ...
           // Run cohort ecology
           RunWithinCellCohortEcologyNoMerge(_CellList[cellIndex][0], _CellList[cellIndex][1], partial, WorkingGridCellCohorts, WorkingGridCellStocks, InitialisationFileStrings["OutputDetail"], cellIndex, initialisation);
        ...
        private void RunWithinCellCohortEcologyNoMerge(uint latCellIndex, uint lonCellIndex, ThreadLockedParallelVariables partial,
            GridCellCohortHandler workingGridCellCohorts, GridCellStockHandler workingGridCellStocks, string outputDetail, int cellIndex, MadingleyModelInitialisation initialisation)
        {
            ...
            MadingleyEcologyCohort.InitializeEcology(EcosystemModelGrid.GetCellEnvironment(latCellIndex, lonCellIndex)["Cell Area"][0],
                 _GlobalModelTimeStepUnit, EcosystemModelGrid.GetCellEnvironment(latCellIndex, lonCellIndex)["Temperature"], DrawRandomly);
        ...
        //ROGIER EDIT:
        // HERE ARE THE TWO FUNCTIONS TO RUN JUST THE MERGE FUNCTION FOR ALL GRID CELLS
        //ROGIERS VERSION, SANS MERGE, OF THE RUN CELL ALGORITHM! REQUIRES 2 FUNCTIONS: RunCellNoMerge AND RunWithinCellCohortEcologyNoMerge 
        public void RunCellOnlyMerge(int cellIndex, ThreadLockedParallelVariables partial, Boolean dispersalOnly,
           MadingleyModelInitialisation initialisation)
        {
            // Apply any climate change impacts
            ClimateChangeSimulator.ApplyTemperatureScenario(
            ...
            // Run cohort ecology
            RunWithinCellCohortEcologyOnlyMerge(_CellList[cellIndex][0], _CellList[cellIndex][1], partial, WorkingGridCellCohorts,  InitialisationFileStrings["OutputDetail"], cellIndex, initialisation);
        ...
        private void RunWithinCellCohortEcologyOnlyMerge(uint latCellIndex, uint lonCellIndex, ThreadLockedParallelVariables partial,
            GridCellCohortHandler workingGridCellCohorts, string outputDetail, int cellIndex, MadingleyModelInitialisation initialisation)
        {
        ...
        public void RunCellsInParallel(MadingleyModelInitialisation initialisation)
        {
            ...
            else
            {
                ...
                {
                    //ROGIER EDIT: 
                    //MAYBE A CLEVER CHANGE WOULD BE TO TAKE THE MERGING ALGORITHM INTO A DISTINCT PARALLEL LOOP 
                    // THEN COULD PROBE INBETWEEN THE TWO!!
                    Console.WriteLine("DOING TERRESTRIAL ECOLOGY WITHOUT MERGE...");
                    // Run a parallel loop over rows
                    
                    Parallel.For(0, _CellList.Count, () => new ThreadLockedParallelVariables { Extinctions = 0, Productions = 0, Combinations = 0, NextCohortIDThreadLocked = NextCohortID }, (ii, loop, threadTrackedDiagnostics) =>
                    {
                        if (EcosystemModelGrid.GetCellEnvironment(_CellList[ii][0], _CellList[ii][1])["Realm"][0] == 1.0) RunCellNoMerge(ii, threadTrackedDiagnostics, initialisation.DispersalOnly, initialisation);
                        return threadTrackedDiagnostics;
                    },
                     (threadTrackedDiagnostics) =>
                     {
                    ...
                    Console.WriteLine("HAVE DONE ECOLOGY BUT NO COHORT MERGING...");
                    Console.WriteLine("NOW COLLECTING tend DATA");
                    dataobj.CollectTendData(_CellList,EcosystemModelGrid,CurrentTimeStep);
                    Console.WriteLine("NOW DOING MERGING...");

                    Parallel.For(0, _CellList.Count, () => new ThreadLockedParallelVariables { Extinctions = 0, Productions = 0, Combinations = 0, NextCohortIDThreadLocked = NextCohortID }, (ii, loop, threadTrackedDiagnostics) =>
                    {
                        if (EcosystemModelGrid.GetCellEnvironment(_CellList[ii][0], _CellList[ii][1])["Realm"][0] == 1.0) RunCellOnlyMerge(ii, threadTrackedDiagnostics, initialisation.DispersalOnly, initialisation);
                        return threadTrackedDiagnostics;
                    },
                     (threadTrackedDiagnostics) =>
                     {
                ...
                else
                {
                    ...
                    Console.WriteLine(initialisation.RunRealm);
                    Console.ReadKey();
                }
            }
        ...
        public void RunCrossGridCellEcology(ref uint dispersals, bool dispersalOnly, MadingleyModelInitialisation modelInitialisation)
        {
            ...
            if (SpecificLocations != true)
            {
                ...
                MadingleyEcologyCrossGridCell.UpdateCrossGridCellEcology(EcosystemModelGrid, ref dispersals, TrackCrossCellProcesses, CurrentTimeStep,dataobj);
            }
